...
%then combine the community information from multiple networks           %
%to get a better community result                                        %
%Author: Kaixian Yu, Ph.D.                                               %
%Department of Biostatistics                                             %
%the University of Texas MD Anderson Cancer Center                       %
%10/02/2016                                                              %
%One may use/distributed/reproduce/modify under GPLv3.                   %
%                                                                        %
%The author of this file is not responsible for any SCORE related issue. %
%------------------------------------------------------------------------%

function combining_information(N,path_to_score,in_file,out_file,Max_size,Min_size,Max_iter,M)
...
%	M             : Number of split from the second round.
	...
	all_nodes       = [];
	max_node        = 0;
	prior_community = cell( 1, length(in_file) );
	for net_id = 1 : length(in_file) % first identify communities for each network
		edge_input      = load(in_file{net_id}); % loading edges
		...
		max_node     = max(max_node,net_s);
		ensp_names   = 1:net_s; 
		actual_nodes = unique(col1);% actual nodes presented in the network
		all_nodes    = unique([all_nodes;actual_nodes]);
		...
		prior_community{net_id} = block_names;
	end
	%then based on the results, conducting a k mean on the spectral of similarity matrix
	% constructing the similarity matrix
	similarity = sparse(max_node,max_node);
	for net_id = 1 : length(in_file)
		for com = 1 : length( prior_community{net_id} )
			similarity(prior_community{net_id}{com}, prior_community{net_id}{com}) = similarity(prior_community{net_id}{com}, prior_community{net_id}{com}) + 1;			
		end
	end
	
	ensp_names     = 1:max_node; 
	size_community = zeros(1,N);% tracking the size of each community
	block_matrix   = cell(1,N);% tracking the adj matrix for each community
	block_names    = cell(1,N);% node names of each community
	tmp            = spectral_kmeans(similarity,N);%first split
	for i = 1:(N)
		ind               = find(tmp == i);
		size_community(i) = length(ind);
		block_matrix{i}   = weighted_adj(ind,ind);
	...
	while max(size_community) > Max_size && iter <= Max_iter
		...
		for i = 1: length(block_names)
			if size_community(i) > Max_size
				S                  = min(M,ceil(size_community(i)/Max_size));
				tmp                = spectral_kmeans(block_matrix{i},S);
				tmp_size_community = zeros(1,S);
				tmp_block_matrix   = cell(1,S);
				tmp_block_names    = cell(1,S);
				for j = 1:S
